...
export function SegmentBuilderDrawer({ open, onOpenChange, onCreate }: SegmentBuilderDrawerProps) {
  ...
  return <Sheet open={open} onOpenChange={handleOpenChange}><SheetContent className="w-full overflow-y-auto bg-slate-50 sm:max-w-[760px]">
    {step === 1 ? <div className="space-y-5 pb-24">
      <SheetHeader><div className="flex items-start justify-between gap-6 pr-8"><div><SheetTitle>Build a segment</SheetTitle><SheetDescription className="mt-1">Build the audience directly with nested AND/OR rule groups.</SheetDescription></div><Button onClick={() => setStep(2)} disabled={!complete}>Review segment</Button></div></SheetHeader>
      <div className="rounded-xl border bg-white p-4"><div className="flex items-center justify-between gap-4"><div><p className="text-sm font-semibold">Match groups when</p><p className="text-xs text-muted-foreground">This is the original nested builder: AND narrows, OR creates alternate paths.</p></div><ConjunctionToggle value={definition.conjunction} onChange={(conjunction) => setDefinition((current) => ({ ...current, conjunction }))} /></div></div>
