...
interface TurnstileProps {
  ...
  className?: string;
  /**
   * Increment this to force the widget to fetch a fresh token.
   * Useful after a failed submit consumed the previous (single-use) token.
   */
  resetKey?: number;
}
...
export function Turnstile({
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      setTimeout(() => {
        if (window.turnstile) {
          ...
          logger.error('Turnstile object not available after script load');
    ...
    script.onerror = (e) => {
      logger.error('Failed to load Turnstile script:', e);
      setError('Failed to load bot protection. Please refresh the page.');
      if (onError) onError();
    };
  ...
  useEffect(() => {
    if (!isLoaded || !containerRef.current || !window.turnstile) {
      if (isLoaded && !window.turnstile) {
        logger.error('Turnstile script loaded but window.turnstile is not available');
      }
      return;
    }
    ...
    if (!siteKey || siteKey.trim() === '') {
      logger.error('Turnstile siteKey is empty or invalid');
      setError('Bot protection is not configured correctly.');
      return;
    }

    logger.debug('Rendering Turnstile widget with siteKey:', siteKey.substring(0, 10) + '...');
    ...
    try {
      const widgetId = window.turnstile.render(containerRef.current, {
        ...
        'timeout-callback': () => {
          logger.warn('Turnstile timeout-callback triggered');
          setError('Verification timed out. Please try again.');
          if (onTimeout) onTimeout();
        },
      });
      logger.info('Turnstile widget rendered successfully, ID:', widgetId);
      widgetIdRef.current = widgetId;
    } catch (err) {
      logger.error('Error rendering Turnstile:', err);
      setError('Failed to initialize bot protection.');
      if (onError) onError();
    }
  }, [isLoaded, siteKey, theme, size, onVerify, onError, onExpire, onTimeout]);

  // Force a fresh token when the parent bumps resetKey (e.g. after a failed
  // submit consumed the single-use token). Skip the initial mount (resetKey 0).
  useEffect(() => {
    ...
    if (widgetIdRef.current && window.turnstile) {
      try {
        ...
        logger.error('Error resetting Turnstile after resetKey change:', e);
      }
    }
  }, [resetKey]);
