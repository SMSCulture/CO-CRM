    ...
    (set, get) => ({
        ...
        try {
          ...
          if (typeof window !== 'undefined') {
            try {
              ...
              logger.debug('🚩 Logout flag set');

              // Clear cookies from client side
              document.cookie = 'token=; Path=/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;';
              document.cookie = 'refreshToken=; Path=/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;';
              logger.debug('🗑️ Client-side cookies cleared');
              ...
              logger.debug('🗑️ localStorage cleared');

            } catch (storageError) {
              logger.error('🗑️ Error clearing storage:', storageError);
            }
          }
          ...
          try {
            ...
            logger.debug('🗑️ Auth state cleared');
          } catch (stateError) {
            logger.error('🗑️ Error clearing state:', stateError);
          }

          // Call logout endpoint and WAIT for response
          // This is critical: httpOnly cookies can only be cleared by the server response
          ...
          try {
            ...
            logger.info('✅ Force-logout response received, cookies should be cleared');
          } catch (error) {
            logger.error('🚪 Logout endpoint error:', error);
          }

          // Redirect to home AFTER receiving the response
          logger.debug('🔀 Redirecting to home...');
          window.location.replace('/');

        } catch (overallError) {
          logger.error('🚪 Overall logout error:', overallError);
          // Emergency redirect
          window.location.replace('/');
        }
      },
      ...
      initializeAuth: async (force = false) => {
        ...
        if (!force && !currentState.isInitializing && currentState.user) {
          logger.debug('Auth already initialized with user, skipping (use force=true to re-initialize)');
          return;
        }

        logger.debug('🔐 Initializing auth...', force ? '(FORCED)' : '');
        ...
        try {
          ...
          logger.debug('🔑 Fetching user data from /api/auth/me...');
          ...
          if (response.ok) {
            ...
            if (!isPublicRoute) {
              logger.warn('Auth initialization failed, user not authenticated');
            }
      ...
      loadUserPermissions: async (companyId?: string) => {
        ...
        if (!isAuthenticated || !user) {
          logger.debug('⏭️ Skipping permission load: user not authenticated');
          return;
        }
        ...
        try {
          ...
          if (errorMessage.includes('PermissionsAPI not initialized') ||
              ...
              logger.warn('⚠️ Apollo Client not ready, skipping permissions for now');
            return;
          }
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        ...
        if (state && typeof window !== 'undefined') {
          ...
          logger.debug('💾 Rehydrating auth state...');

          // Safety check: if we're on home page and isLoggingOut is true, it's a bug
          const isHomePage = window.location.pathname === '/';

          if (isHomePage && state.isLoggingOut) {
            logger.error('🐛 BUG DETECTED: isLoggingOut is true on home page, forcing complete reset');
            state.user = null;
          ...
          if (logoutFlag === 'true') {
            logger.debug('🚩 Logout flag detected, forcing empty state');
          ...
          logger.debug('💾 State rehydrated, AuthInitializer will verify with server');
