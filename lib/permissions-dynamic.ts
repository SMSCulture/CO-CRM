...
export function debugUserPermissions(user: User | null | undefined): void {
  ...
  logger.debug(`🎭 Role: ${user.role?.name || 'No role'}`);
  logger.debug(`📊 Permission Level: ${getUserPermissionLevel(user)}`);
  logger.debug(`🔑 Permissions (${user.permissions?.length || 0}):`, user.permissions || []);
  ...
  if (user.permissions) {
    ...
    logger.debug('📋 Permissions by resource:', resourceMap);
  }
}