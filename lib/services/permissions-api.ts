...
export class PermissionsAPI {
  ...
  static async getMyPermissions(companyId?: string): Promise<string[]> {
    // Defensive guard: ensure client is initialized
    let client;
    try {
      client = this.getClient();
    } catch {
      logger.warn('PermissionsAPI client not initialized yet, skipping permission fetch.');
      return [];
    }

    // Defensive guard: ensure query document is available
    if (!GET_MY_PERMISSIONS) {
      logger.warn('GET_MY_PERMISSIONS query is missing. Skipping permission fetch.');
      return [];
    }
    ...
    try {
      ...
      // Handle Apollo InvariantError (client not ready / invalid query) gracefully
      const msg = error instanceof Error ? error.message : String(error);
      logger.warn('PermissionsAPI.getMyPermissions failed:', msg);
      return [];
    }
  }
  ...
  static async getUserPermissions(userId: string, companyId?: string): Promise<string[]> {
    ...
    try {
      ...
      logger.warn('PermissionsAPI client not initialized yet, skipping user permission fetch.');
      return [];
    }

    if (!GET_USER_PERMISSIONS) {
      logger.warn('GET_USER_PERMISSIONS query is missing. Skipping user permission fetch.');
      return [];
    }
    ...
    try {
      ...
      const msg = error instanceof Error ? error.message : String(error);
      logger.warn('PermissionsAPI.getUserPermissions failed:', msg);
      return [];
    }
  }
  ...
  static async getGlobalRoles(): Promise<Role[]> {
    ...
    try {
      ...
      logger.error('Error fetching global roles:', error);
      return [];
    }
  }
  ...
  static async getCompanyRoles(companyId: string): Promise<Role[]> {
    ...
    try {
      ...
      logger.error('Error fetching company roles:', error);
      return [];
    }
  }
  ...
  static async currentUserHasPermission(
    ...
    try {
      ...
      logger.error('Error checking current user permission:', error);
      return false;
    }
  }
  ...
  static clearPermissionsCache(): void {
    ...
    try {
      ...
      logger.error('Error clearing permissions cache:', error);
    }
  }
  ...
  static async refreshUserPermissions(companyId?: string): Promise<string[]> {
    ...
    try {
      ...
      logger.warn('PermissionsAPI client not initialized yet, skipping refresh.');
      return [];
    }

    if (!GET_MY_PERMISSIONS) {
      logger.warn('GET_MY_PERMISSIONS query is missing. Skipping refresh.');
      return [];
    }
    ...
    try {
      ...
      const msg = error instanceof Error ? error.message : String(error);
      logger.warn('PermissionsAPI.refreshUserPermissions failed:', msg);
      return [];
    }
  }
}